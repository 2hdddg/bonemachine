...
module.exports.create = function(registry, port_range){
    ...
    function start(callback){
        get_services(function(error, services){
            if (error){
                callback(error);
                return;
            }
    ...
    function get_free_port(services){
    ...
    function allocate(allocation, allocate_callback){
        var registration = _.cloneDeep(allocation);

        async.waterfall([
            get_services,
            function (services, callback){
                registration.port = get_free_port(services);
                registry.register(registration, callback);
            }
        ], function(error, result){
            allocate_callback(error, result);
        });
    }
